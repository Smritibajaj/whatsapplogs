...
const messages = [];
async function getChatData(id, offset) {
    const data = await axios.get(`https://us-central1-visof-new-vuja.cloudfunctions.net/middleware/conv/${id}`, {
        params: {
            limit: 20,
            offset: offset,
        },
        ...
        if (type === "list_reply") {
            return {
                ...
                body: {
                    ...
                    description: data?.messages?.[0]?.interactive?.list_reply?.description,
                },
            };
        }
        if (type === "nfm_reply") {
            return {
                ...
                body: data?.messages?.[0]?.interactive?.nfm_reply?.response_json
                    ? JSON.parse(data?.messages?.[0]?.interactive?.nfm_reply?.response_json)?.flow_token
                    : "",
            };
        }
    }
...
const getTimeStamp = (datum) => {
    const data = datum?.request?.entry?.[0]?.changes?.[0]?.value;
    if (data?.messages?.[0]?.timestamp)
        return data?.messages?.[0]?.timestamp;
};
export async function getMessages(id, offset) {
    const totalMessagesLength = messages.length;
    let randomNumber = Math.floor(Math.random() * totalMessagesLength);
    if (randomNumber > totalMessagesLength)
        randomNumber = totalMessagesLength;
    if (randomNumber === 1)
        randomNumber = 2; // so we always have atleast 1-2 messages.
