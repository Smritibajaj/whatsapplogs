...
import { ButtonBox, ChatMessage, ChatMessageFiller, ChatMessageFooter, Container, Date, DateWrapper, EncryptionMessage, MessageGroup, MainBox, } from "./styles";
...
export default function MessagesList(props) {
    ...
    const { containerRef, lastMessageRef } = useScrollToBottom(onShowBottomIcon, shouldScrollToBottom, params.id);
    const getMsgs = async (offset) => {
        if (params.id) {
            try {
...
const SingleMessage = forwardRef((props, ref) => {
    const { message } = props;
    ...
    return (<>
      <ChatMessage key={message.id} className={message.isOpponent ? "chat__msg--received" : "chat__msg--sent"} ref={ref}>
        <Box p={2} display={"flex"} position={"relative"} flexDirection={"column"}>
          {normalMessage.includes(message.type) && <span>{message.body}</span>}
          {message.type === "list_reply" && (<div>
              <div>{message.body.title}</div>
              <div>{message.body.description}</div>
            </div>)}
          {message.baseType === "response" && message.type === "button" && (<div>
              {message?.body?.header?.type === "image" && (<img src={message.body.header?.image?.link} width="100%"/>)}
              <div>{message.body.body.text}</div>
            </div>)}
          {message.baseType === "response" && message.type === "list" && (<div>
              <div>{message.body.body.text}</div>
            </div>)}
          <ChatMessageFiller />
          <ChatMessageFooter>
            <span>{message.timestamp}</span>
            {message.actor === "user" && (<Icon id={`${message.messageStatus === "SENT" ? "singleTick" : "doubleTick"}`} className={`chat__msg-status-icon ${message.messageStatus === "READ"
                ? "chat__msg-status-icon--blue"
                : ""}`}/>)}
          </ChatMessageFooter>
        </Box>
        {message.baseType === "response" && message.type === "list" && (<Box sx={{
            ...
            <Button sx={{ width: "100%" }} onClick={() => {
              ...
              <FormatListBulletedIcon sx={{ fontSize: "12px", mx: 1 }} color="primary"/>
              {message.body?.action?.button}
            </Button>
          </Box>)}
      </ChatMessage>
      {message.baseType === "response" && message.type === "button" && (<MainBox direction={"row"} gap={1} width={"100%"}>
          {message.body.action?.buttons?.map((button) => {
                return (<ButtonBox>
      ...
      <MessageDialog selectedValue={selectedValue} open={open && message.body.action?.sections?.length > 0} onClose={handleClose}/>
    </>);
});
